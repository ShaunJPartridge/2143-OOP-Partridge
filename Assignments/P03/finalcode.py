...
class Person(pygame.sprite.Sprite):
    ...
    def __init__(self,
        ...
        speeds = range(1,6)# in simulation class??
...
class Simulation():
    ...
    def __init__(self):
        ...
        self.population_count = len(self.people)
        self.duration_of_illness = 1
    ...
    def collide(self, other,social_distance=20):# put in sumulation class??
        ...
        x1, y1 = P.rect.center   #declare Person instance and use setter & getter to get values
        ...
        if d < P.width*1 and random.random() < self.infection_rate: # all were self.something
            P.image = pygame.image.load(Config.sprite_images["red"])
            P.image = pygame.transform.scale(P.image,
                                                (P.width, P.height))
            P.state = "infected"
    
    def RunSimulation(self,social_distancing,infection_radius,infection_rate,population_count,duration_of_illness):
...
class Community(Population):
    ...
    def __init__(self):
        ...
        self.p = Person()
        self.num_people_socialdistancing = 0
        self.community_size = 0

    def Community(self,p):
        Community.append(p)
        community_size += 1
...
if __name__=='__main__':
    ...
    S = Simulation()
    # loop N times
    for i in range(S.population_count):
